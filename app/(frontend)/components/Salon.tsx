...
const DEFAULT_SALON_PARAGRAPHS = [
  "Dort, wo die Lübecker Altstadt in die Kronsforder Allee übergeht, liegt das Haarstudio Anetta — mit dem Mühlentor vor der Tür und dem Viertel im Herzen.",
  "Ein Salon zum Wohlfühlen: herzlich, persönlich und unkompliziert. Alle Generationen sind willkommen — von der ersten Ponyfrisur bis zur Dauerwelle.",
];
...
type SalonProps = {
  ...
  headingLines?: string[];
  paragraphs?: string[];
  storefrontImage?: { src: string; alt: string };
...
export function Salon({
  eyebrow = "Der Salon",
  headingLines = ["Am Mühlentor,", "mitten in", "Lübeck."],
  paragraphs = DEFAULT_SALON_PARAGRAPHS,
  storefrontImage = {
        ...
        <div className="flex flex-col items-start">
          ...
          <Reveal delay={0.15} className="mt-8 max-w-[46ch]">
            {paragraphs.map((text, i) => (
              <p
                key={i}
                className={`${i === 0 ? "" : "mt-5 "}hyphens-auto text-pretty text-[16px] font-medium leading-[1.65] text-foreground/80 sm:text-[17px]`}
              >
                {text}
              </p>
            ))}
